...
def sequencer_server(request):
    ss = SequencerServer(0, publisher)
...
@pytest.fixture()
def ss2(request):
    ss = SequencerServer(1, publisher)
    path = os.path.split(__file__)[0]
...
def test_run(sequencer_server, protocol, mock_responses):
    ...
    end_req = te_protocol.create_request('end_test', None, '', 5000)
    ...
    expected = []
    expected.append(mock.call(start_req, 5000))
...
def test_exception_run(ss2, protocol, mock_responses):
    te_protocol = TERPCProtocol()
    m = mock.MagicMock()
    m.side_effect = mock_responses
    ...
    assert not hasattr(response, 'error'), response.error
    ss2.sequencer.wait(2)
    assert m_report.mock_calls[-1] == mock.call(events.SEQUENCE_END, result=False, logs='6000')
...
def test_step(sequencer_server, protocol):
    ...
    #thee should be no problem serialize this
    response.serialize()
    req = protocol.create_request('step', None)
    response = ss.rpc_server.handle_message(0, req.serialize())
    assert not hasattr(response, 'error'), response.error
...
def test_timeout(sequencer_server, protocol, delay_mock):
    ...
    response = ss.rpc_server.handle_message(0, req.serialize())
    assert not hasattr(response, 'error')
    assert response.result == True #this should have timedout
    req = protocol.create_request('abort', None)
    response = ss.rpc_server.handle_message(0, req.serialize())
    assert not hasattr(response, 'error')
    ...
    response = ss.rpc_server.handle_message(0, req.serialize())
    assert not hasattr(response, 'error')
    assert response.result == False #this should not have timedout
