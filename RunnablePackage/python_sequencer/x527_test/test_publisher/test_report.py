...
def sequencer_server(request):
    ...
    plan_path = os.path.join(path, '../test_sequencer/short_plan__1.0.csv')
...
def test_complete_run(sequencer_server, mock_responses):
    ...
    assert r_data[0].data['version'] == '1.0_' + sequencer.__version__
...
def test_engine_exception_run(sequencer_server, mock_responses):
    ...
    assert r_data[5].data.has_key('error')
