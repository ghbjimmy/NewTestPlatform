...
def test_read_statio_info(station_info):
...
def test_commit_run(station_info):
    ...
    spec = IPTestSpec(u'Parametric data test', subtest_name=u'fail_test', limits={'low_limit':0, 'high_limit':8}, unit='ly')
...
def test_name_too_long(uut):
    ...
    with pytest.raises(InstantPuddingError) as exc:
        spec = IPTestSpec('passing_test', subtest_name='passing sub test', subsubtest_name=long_name, limits={'low_limit':0, 'high_limit':9}, unit='ly')
    assert exc.value.message.startswith("subsubtest name is too long")

    long_name = 'n' * (pypudding.PDCA_UNIT_MAX_LEN + 1)
    with pytest.raises(InstantPuddingError) as exc:
        spec = IPTestSpec('passing_test', subtest_name='passing sub test', limits={'low_limit':0, 'high_limit':9}, unit=long_name)
