...
-- @alias tcp_module
print("< ".. tostring(CONFIG.ID).." > Load Fixture TCP Socket...")
...
function tcp_module._Arm_Socket_Connect_()
  os.execute("ping -c 5 "..CONFIG.FIXTURE_ADDRESS[CONFIG.ID + 1]);	--ping device
  print("< ".. tostring(CONFIG.ID).." fixture.socket.lua > Fixture Port connect to: " .. CONFIG.FIXTURE_ADDRESS[CONFIG.ID + 1] .. ":" .. tostring(CONFIG.FIXTURE_PORT) .. "...")
  tcp_module.InstrumentSocket = assert(
    socket.timeout_connect(5, CONFIG.FIXTURE_ADDRESS[CONFIG.ID + 1], CONFIG.FIXTURE_PORT),"HW: "..tostring(CONFIG.ID).." Connect Fail")
  tcp_module.InstrumentSocket:settimeout(10)
  print("< ".. tostring(CONFIG.ID).." fixture.socket.lua > Fixture Port connect successfully")
end
...
function tcp_module._ARM_Send_Cmd_(command)
  ...
  if(HwLogOut) then HwLogOut(s1 .."<    send: > " .. tostring(command)) end
  command = command .. "\r\n"
  ...
  if(rcvState == "timeout") then error("ARM Response Timeout") end
  if(rcvState == "closed") then error("ARM Port connection closed.") end
...
function tcp_module._ARM_Socket_Receive_()
    return tcp_module.InstrumentSocket:receive()
end
...
--_Arm_Socket_Connect_();

print("< ".. tostring(CONFIG.ID).." > Finished Load Fixture  TCP Socket...\r\n")
