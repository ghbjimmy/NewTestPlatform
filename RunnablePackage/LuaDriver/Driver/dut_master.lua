...
local address = config_utils.get_addr(CONFIG, "UART_ZMQ_REQ", CONFIG.ID)
print("< " ..tostring(CONFIG.ID).. " dut_master.lua > Connect DUT Command REP : ",address)
local serial_cmd_zmq, err = context:socket(zmq.REP, {bind = address})
zassert(serial_cmd_zmq, err)

local address = config_utils.get_addr(CONFIG, "DUT_UART_ZMQ", CONFIG.ID)
print("< " ..tostring(CONFIG.ID).. " dut_master.lua > Connect DUT Output PUB : ",address)
local dut_serial_output_zmq, err = context:socket(zmq.PUB, {bind = address})
zassert(dut_serial_output_zmq, err)
print("< " ..tostring(CONFIG.ID).. " > Connect to port Success")
...
if CONFIG.SERIAL_PORT == true then
    ...
    print("< " ..tostring(CONFIG.ID).. " dut_master.lua > Connect to port " ..CONFIG.DUT_UART_ADDRESS[CONFIG.ID + 1]..":"..CONFIG.DUT_UART_PORT)
    dut_socket = assert(
    ...
    print("< " ..tostring(CONFIG.ID).. " dut_master.lua > Connect to port Success")
end
...
local function serial_loop()	
    ...
    -- if recv_buffer:match("\n") or recv_buffer:match("\r") then
        -- print("Publisher :",dut_serial_output_zmq:send(recv_buffer..'\0'))
    ...
    if time_utils.get_time_delta_ms(start_time) > max_send_timeout then
        dut_serial_output_zmq:send(recv_buffer..'\0')
    ...
    if msg ~= nil then
        ...
        dut_socket:send(string.format("%s\r\n", command))
